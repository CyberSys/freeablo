...
namespace FAGui
{
    CharacterDialoguePopup::CharacterDialoguePopup(GuiManager& guiManager) : mGuiManager(guiManager)
    {
    }
    ...
    CharacterDialoguePopup::UpdateResult CharacterDialoguePopup::update(struct nk_context* ctx)
    {
        ...
        auto boxTex = renderer->loadImage(wide ? "data/textbox.cel" : "data/textbox2.cel");
        ...
        auto dialogRectangle = nk_rect(screenW / 2.0f - (wide ? boxTex->getWidth() / 2 : 0.0f),
            ...
            [&]()
        {
            ...
            for (const auto& line: mIntroduction)
            ...
            {
                if (mDialogMenu.update(ctx, mDialogOptions, mScroll) == MouseAndClickMenu::Result::Activated)
                    result = mDialogActions[mDialogMenu.getSelectedIndex()]();
            }
